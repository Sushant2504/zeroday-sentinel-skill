...
#### Step 2: JSON Report Data (for Live Dashboard)

Write the report data as JSON to `/tmp/groundwork-report.json` with this schema:

```json
{
  "project_name": "<project name>",
  "scan_date": "<YYYY-MM-DD HH:MM>",
  "scan_mode": "<Full Project Audit (Groundwork Enhanced) | Pending Changes Review | ...>",
  "tech_stack": "<detected languages, frameworks, tools>",
  "overall_risk": "<CRITICAL | HIGH | MEDIUM | LOW | CLEAN>",
  "overall_risk_class": "<critical | high | medium | low | clean>",
  "files_reviewed": <number>,
  "endpoints_count": <number>,
  "findings_total": <number>,
  "sections": {
    "executive_summary": "<HTML content for executive summary>",
    "architecture": "<HTML content for architecture overview>",
    "code_patterns": "<HTML content for code patterns>",
    "api_surface": "<HTML content for API surface>",
    "data_models": "<HTML content for data models>",
    "dependencies": "<HTML content for dependency analysis>",
    "devops": "<HTML content for DevOps & infrastructure>",
    "git_health": "<HTML content for git & repository health>",
    "security_findings": "<HTML content for security findings>",
    "supply_chain": "<HTML content for supply chain analysis>",
    "adversarial": "<HTML content for adversarial analysis>",
    "doc_correlation": "<HTML content for documentation correlation>",
    "cross_project": "<HTML content for cross-project overlap>",
    "remediation": "<HTML content for remediation summary>",
    "verification": "<HTML content for verification report>",
    "security_posture": "<HTML content for security posture>"
  }
}
```

Each section value should contain rendered HTML (tables, code blocks, finding cards). Use the `.finding` div structure with `data-severity` attributes for findings so severity filtering works in the dashboard.

#### Step 3: Inform the User

Tell the user:
- Static report available at `/tmp/groundwork-report.html`
- For a live dashboard, start the server: `python3 scripts/serve-dashboard.py`
- Dashboard will be available at `http://localhost:8450`
- If the dashboard server is already running, it will auto-refresh with the new data
